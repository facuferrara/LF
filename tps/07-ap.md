...
1. Investigue y justifique L = {a^n b^n / n ≥ 1} no es un LR. Demuestre que las producciones de la siguiente gramática generan al lenguaje L: S -> aSb | ab



2. Sea la transición T(4, λ, Z) = {(5, RP)}. Describa qué significa

    Estado actual: 4

    Entrada: no se consume ningún símbolo (avance sin leer).

    Tope de pila: debe ser Z.

    Acción:

    Desapilar Z.

    Apilar R y luego P (en ese orden, es decir, R queda debajo de P, siendo P el nuevo tope).
...
2. {a^(n+1)b^n / n ≥ 1}
3. {xyxy^nzy^(2n+1) / n ≥ 0}
4. {a^nb^(2n)a^t/ n ≥ 1, t ≥ 0}
5. {a^nb^mc^(m+n) / m, n ≥ 0}
6. {a^n b^m c^p a^q b^n / q = p + m, n, m ≥ 1, p ≥ 0}
    p+m= despues de cada bc que pones tiene que haber una a
    puede haber una b o muchas
    La c puede estar o no xq p>=0
    
    L = { abab, abcaab, aabcaaabb, abbcaaab, aabbccaaaabb ....}
    ...
    Z        abcaab     δ(q0, a, Z ) = δ(q0, XB)
    BX       bcaab      δ(q0, b, X ) = δ(q0, CA)
    BAC       caab      δ(q0, c, C ) = δ(q0, A)
    BAA       aab       δ(q0, a, A ) = δ(q0, lamda)
    BA         ab       δ(q0, a, A ) = δ(q0, lamda)
    B           b       δ(q0, b, B ) = δ(q0, lamda)
    lamda      lamda    Accept



7. {a^nb^mc^pa^qb^rc^s/ q = p + m, 2n = r + s, n, m ≥ 1, p ≥ 0}
8. {wa^rb^rw^(-1) / w ∈ {a, b}*, r ≥ 1}.
9.  {(ab)^(2s+1) c^j (d)^n e^(t+2n) / s, t, n ≥ 0 y j > t} U {(ab)^(h+1) d^(2k) e^(k+i) / h, i ≥ 0; k ≥ 1}
10. {a^(2k)b^(2n)c^kd^j / k, n, j ≥ 0}
11. {x^ry^sz^t / t = r+s y r, s ≥ 1}
12. {x^ry^sz^t / s = r+t y r, s ≥ 1}
13. {x / x = a Y e donde Y = b^(3n)cd^(3n), n ≥ 1}
14. {1^n0^k / n ≥ 0 y k = 3n}
